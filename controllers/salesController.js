...
const generateSalesReport = async (query) => {
    try {
        ...
        const formattedOrders = orders.map(order => ({
            ...
            totalAmount: order.orderedItems.reduce((sum, item) => sum + (item.priceAtPurchase * item.quantity), 0),
            discount: order.orderedItems.reduce((sum, item) => sum + (item.totalDiscount || 0), 0),
